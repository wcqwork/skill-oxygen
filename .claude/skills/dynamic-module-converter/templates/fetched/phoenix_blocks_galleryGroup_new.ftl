<div class="backstage-blocksEditor-wrap wra block-39544" data-block-uuid="groupGallery"  data-gjs-type="developer-node-component" data-show-helperel="1" data-dynamic-toolbar="1"  data-block-type="phoenix_blocks_galleryGroup_new" data-default-setting={"dataType":"0","dataGroupId":[],"jumpMethod":"0","translationEntry":[]}>
...
query='{
      ...
      <ul class="block-product-group-list block-common-group-list ${pcDefaultStyle} ${padDefaultStyle} ${phoneDefaultStyle} ${addBlockKeyClass} 22222222222222" data-closeway="${closeWay}">
          ...
          [#if data?? && data.galleryCateList?? && (data.galleryCateList?size > 0)]
              [#list data.galleryCateList as group]
                  <li class="group-item-li grouplist-li-first [#if galleryCateId?? && galleryCateId == group.cateId ]current[/#if]" data-pid="${group.encodeId}">
                      ...
                      [#if group.subCates?? && group.subCates?has_content]
                          ...
                          [#list group.subCates as secondGroup]
                              <li class="grouplist-li-two group-item-li grouplist-li-other [#if galleryCateId?? && galleryCateId == secondGroup.cateId ]current[/#if]">
                                  ...
                                  [#if secondGroup.subCates??]	
                                      <ul class="three-classify classify-item">
                                          [#list secondGroup.subCates as thirdGroup]
                                              <li class="grouplist-li-three group-item-li grouplist-li-other [#if galleryCateId?? && galleryCateId == thirdGroup.cateId ]current[/#if]">
    ...
    <script>
          $(function(){
              window._block_namespaces_['galleryGroup39544'].init({'relationId':'${relationId}','relationType':'${relationType}','pageId':'${pageId}','nodeId':'groupGallery_${nodeId!""}', 'appId': '${appId!}', 'appIsDev': '${appIsDev!"0"}','appVersion':'${appVersion}'});
          });
      </script>
[/@api]
