<div class="block35704" data-gjs-type="phoenix-container" data-strong="1">
    ...
    [#if __current_site_lanCode__?? && specialLanCode?seq_contains(__current_site_lanCode__)]
        <style data-collect='1'>
            .block35704 .ArticlePicList_ItemContentInnerBox {
                text-align: right;
            }
            @media screen and (min-width: 1200px) {
                div.block35704 .textBox {            
                    margin-right: 16%;
                    margin-left: auto;
                }
            } 
    ...
    <div data-block-uuid="articlelist" data-gjs-type="developer-node-component"
        ...
        <div class="wrapper">
                        ...
                        <div class="ArticlePicList_ItemContent textBox">
                            <div class="ArticlePicList_ItemContentInner textWra">
                                <div class="ArticlePicList_ItemContentInnerBox">
                                    ...
                                    <div class="timeBox">
                                        <div class="name paragraph2">${article.cateName!''}</div>
                                        [#if showDate && showDate == 1]
                                        <div class="time">
                                            <time>
                                                <span class="paragraph2">${article.publishTime?date("yyyy-MM-dd")?string('yy')}-${article.publishTime?date("yyyy-MM-dd")?string('MM')}-${article.publishTime?date("yyyy-MM-dd")?string('dd')}</span>
                                            </time>
                                        </div>
                                        [/#if]
                                    </div>
        	...
        	<script>
                $(function(){
                    window._block_namespaces_['block35704'].init({'relationId':'${relationId}','relationType':'${relationType}','pageId':'${pageId}','nodeId':'articlelist_${nodeId!""}', 'appId': '${appId!}', 'appIsDev': '${appIsDev!"0"}','appVersion':'${appVersion}'});
                });
            </script>
