<div class="backstage-blocksEditor-wrap wra block-39524" data-block-uuid="groupDownload"  data-gjs-type="developer-node-component" data-show-helperel="1" data-dynamic-toolbar="1"  data-block-type="phoenix_blocks_downloadGroup_new" data-default-setting={}>
...
query='{
      ...
      <ul class="block-downloadGroup-container-replace block-common-group-list ${pcDefaultStyle} ${padDefaultStyle} ${phoneDefaultStyle} ${addBlockKeyClass} 22222222222222" data-closeway="${closeWay}">
          <!-- 特殊标识 用于后台保存样式 -->
          ...
          [#if data?? && data.downloadCateList?? && (data.downloadCateList?size > 0)]
              [#list data.downloadCateList as group]
                  <li class="group-item-li grouplist-li-first [#if downloadCateId?? && group.cateId?? && downloadCateId == group.cateId ]current[/#if]" data-pid="${group.encodeId}">
                      ...
                      [#if group.subCates?? && group.subCates?has_content]
                          ...
                          [#list group.subCates as secondGroup]
                              <li class="grouplist-li-two group-item-li grouplist-li-other [#if downloadCateId?? && secondGroup.cateId?? && downloadCateId == secondGroup.cateId ]current[/#if]">
                                  ...
                                  [#if secondGroup.subCates??]	
                                      <ul class="three-classify classify-item">
                                          [#list secondGroup.subCates as thirdGroup]
                                              <li class="grouplist-li-three group-item-li grouplist-li-other [#if downloadCateId?? && thirdGroup.cateId?? && downloadCateId == thirdGroup.cateId ]current[/#if]">
    ...
    <script>
          $(function(){
              window._block_namespaces_['downloadGroup39524'].init({'relationId':'${relationId}','relationType':'${relationType}','pageId':'${pageId}','nodeId':'groupDownload_${nodeId!""}', 'appId': '${appId!}', 'appIsDev': '${appIsDev!"0"}','appVersion':'${appVersion}'});
          });
      </script>
[/@api]
